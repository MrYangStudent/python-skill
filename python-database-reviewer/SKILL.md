...
| 重叠领域 | 本技能关注 | 其他技能关注 |
|---------|----------|-----------|
| SQL 注入防护 | 安全的 SQL 编码模式（参数化查询、白名单） | `python-security-reviewer` 负责攻击面识别 |
| 数据库性能 | N+1 查询、全表扫描、批量操作 | `python-performance-reviewer` 关注通用性能优化 |
| 连接资源 | 连接池配置和连接生命周期管理 | `python-error-handling-reviewer` 关注通用资源泄漏 |
...
# ┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛
#
# 场景：审查 Python 代码中数据库操作的正确性和性能
# 输入：项目代码文件（DAO/Repository/Model 层）
# 输出：数据库审查报告（按严重程度分级）
#
# 审查步骤：
#   1. 检查连接池配置和生命周期管理
#   2. 检查事务处理（BEGIN/COMMIT/ROLLBACK）
#   3. 识别 N+1 查询和全表扫描
#   4. 检查资源关闭（cursor/connection）
#   5. 检查 SQL 注入防护
#
# 常见问题模式：
#   - 连接未关闭: → 使用 with 语句
#   - N+1 查询: → JOIN 或批量查询
#   - SQL 拼接: → 参数化查询 + 白名单
#   - 连接池过小: → 调整 pool_size/max_overflow
#   - 缺少索引: → EXPLAIN 分析 + 添加索引
#
# AI-Usage-End
```
