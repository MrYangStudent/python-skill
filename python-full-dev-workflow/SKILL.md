---
name: python-full-dev-workflow
description: >
  Python 完整开发工作流。当用户要求运行完整开发流程、执行全部步骤、
  或触发 Python 开发工作流时使用。整合 17 个专项技能的全链路开发流程。
triggers:
  - 运行开发工作流
  - 执行完整流程
  - Python 开发流程
  - full workflow
  - 完整开发流程
---

# Python 完整开发工作流

## 角色定义

你是 Python 项目工程化专家，精通从需求分析到生产部署的全流程工程实践。

## 核心原则

1. **质量前置** - 审查先于实现，测试先于交付
2. **渐进式开发** - 小步快跑，持续验证
3. **全面覆盖** - 涵盖代码、测试、安全、性能、文档
4. **可自动化** - 每个步骤都可重复执行

---

## 整合技能列表

本工作流整合以下 17 个专项技能：

| 阶段 | 技能 | 职责 |
|------|------|------|
| 治理 | `python-project-rules` | 初始化、进度同步、README 联动 |
| 文档 | `python-doc-generator` | 项目文档、Docstring 注释 |
| 开发 | `python-feature-development-workflow` | 需求拆解、TDD 开发 |
| 测试 | `python-test-generator` | 单元测试、Mock 编写 |
| 代码审查 | `python-error-handling-reviewer` | 错误处理审查 |
| 代码审查 | `python-concurrency-reviewer` | 并发安全审查 |
| 代码审查 | `python-dependency-reviewer` | 依赖管理审查 |
| 代码审查 | `python-performance-reviewer` | 性能审查 |
| 代码审查 | `python-security-reviewer` | 安全审查 |
| 代码审查 | `python-database-reviewer` | 数据库审查 |
| 代码审查 | `python-logging-reviewer` | 日志规范审查 |
| 代码审查 | `python-typing-reviewer` | 类型注解审查 |
| 代码审查 | `python-api-design-reviewer` | API 设计审查 |
| 代码审查 | `python-refactor-reviewer` | 代码重构审查 |
| 代码审查 | `python-config-reviewer` | 配置管理审查 |
| 代码审查 | `python-architecture-reviewer` | 架构设计审查 |
| 文档 | `python-api-doc-generator` | OpenAPI 文档生成 |
| 部署 | `python-ci-cd-workflow` | CI/CD 流水线配置 |

---

## 完整工作流程

```
┌─────────────────────────────────────────────────────────────────┐
│                      阶段零：项目治理                            │
├─────────────────────────────────────────────────────────────────┤
│  python-project-rules                                            │
│  - 对话初始化（读取 README.md、project.md）                      │
│  - 进度同步（project.md 看板）                                   │
│  - README 联动（架构变更自动同步）                              │
│  - 提交前检查（README 一致性验证）                               │
│  - AI 行为自检                                                  │
└─────────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────────┐
│                      阶段一：准备与文档                           │
├─────────────────────────────────────────────────────────────────┤
│  python-feature-development-workflow  →  需求澄清、任务拆解      │
│  python-doc-generator          →  项目结构、模块文档             │
└─────────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────────┐
│                      阶段二：代码实现                             │
├─────────────────────────────────────────────────────────────────┤
│  代码编写 (遵循开发规范)                                          │
│  - Docstring 注释                                                │
│  - 异常处理                                                      │
│  - TypeHint 类型注解                                              │
│  - 日志记录                                                      │
└─────────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────────┐
│                      阶段三：测试生成                             │
├─────────────────────────────────────────────────────────────────┤
│  python-test-generator                                           │
│  - 正常路径测试                                                  │
│  - 边界情况测试                                                  │
│  - 错误路径测试                                                  │
│  - 异步测试                                                      │
└─────────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────────┐
│                      阶段四：质量审查                             │
├─────────────────────────────────────────────────────────────────┤
│  python-error-handling-reviewer →  错误处理                     │
│  python-concurrency-reviewer    →  并发安全                     │
│  python-dependency-reviewer     →  依赖管理                     │
│  python-performance-reviewer    →  性能表现                     │
│  python-security-reviewer       →  安全漏洞                     │
│  python-database-reviewer       →  数据库操作                   │
│  python-logging-reviewer        →  日志规范                     │
│  python-typing-reviewer         →  类型注解                     │
│  python-api-design-reviewer     →  API 设计                     │
│  python-refactor-reviewer       →  代码重构                     │
│  python-config-reviewer         →  配置管理                     │
│  python-architecture-reviewer   →  架构设计                     │
└─────────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────────┐
│                      阶段五：文档生成                             │
├─────────────────────────────────────────────────────────────────┤
│  python-api-doc-generator                                        │
│  - OpenAPI 3.0 规范                                              │
│  - Postman 集合                                                  │
│  - curl 命令示例                                                 │
│  - API 参考文档                                                   │
└─────────────────────────────────────────────────────────────────┘
                              ↓
┌─────────────────────────────────────────────────────────────────┐
│                      阶段六：CI/CD 与部署                      │
├─────────────────────────────────────────────────────────────────┤
│  python-ci-cd-workflow                                            │
│  - 代码检查（Ruff + Black + isort）                             │
│  - 类型检查（Mypy --strict）                                    │
│  - 测试运行（pytest + 覆盖率门禁）                              │
│  - 安全扫描（pip-audit + safety）                               │
│  - Docker 构建 + 部署                                           │
│  - API 端点验证                                                 │
└─────────────────────────────────────────────────────────────────┘
```

---

## AI 使用示例

```python
# AI-Usage-Begin
# ┏━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┓
# ┃  AI 使用示例：完整开发工作流                            ┃
# ┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛
#
# 场景：执行从需求到部署的 Python 完整开发流程
# 输入：功能需求或项目路径
# 输出：完整交付物（代码 + 测试 + 审查报告 + 文档）
#
# 工作流程（6 阶段）：
#   0. 项目治理：初始化上下文、同步进度
#   1. 准备与文档：需求澄清、任务拆解、项目文档
#   2. 代码实现：Docstring + 异常处理 + 类型注解 + 日志
#   3. 测试生成：正常路径 + 边界 + 错误路径 + 异步
#   4. 质量审查：12 大专项审查（错误/并发/安全/性能/数据库/日志/类型/API/依赖/重构/配置/架构）
#   5. 文档生成：OpenAPI + Postman + curl 示例
#   6. CI/CD 与部署：代码检查 + 类型检查 + 测试 + 安全扫描 + Docker + 部署
#
# AI-Usage-End
```

---

## 触发词

- "运行开发工作流"
- "执行完整流程"
- "Python 开发流程"
- "full workflow"
- "完整开发流程"
