...
| 重叠领域 | 本技能关注 | 其他技能关注 |
|---------|----------|-----------|
| 异常日志 | 日志记录规范和格式化 | `python-error-handling-reviewer` 关注异常处理逻辑 |
| 日志脱敏 | 敏感信息在日志中的屏蔽 | `python-security-reviewer` 关注整体数据安全 |
...
# ┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛
#
# 场景：审查 Python 代码中日志记录的规范性和有效性
# 输入：项目代码文件
# 输出：日志规范审查报告（按严重程度分级）
#
# 审查步骤：
#   1. 扫描所有 print 语句和 logging 调用
#   2. 检查日志级别是否合理
#   3. 检查敏感信息是否脱敏
#   4. 检查日志格式化和结构化日志
#   5. 检查日志配置和轮转
...
#   - 使用 print: → 改用 logging
#   - 敏感信息明文: → 脱敏处理
#   - 字符串拼接日志: → 使用 % 或 f-string 延迟格式化
#   - 日志级别不当: → DEBUG/INFO/WARNING/ERROR/ERROR 分级使用
#   - 缺少 request_id: → 添加上下文追踪
#
# AI-Usage-End
```
