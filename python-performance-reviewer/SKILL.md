...
| 重叠领域 | 本技能关注 | 其他技能关注 |
|---------|----------|-----------|
| 异步代码性能 | 串行改并行、I/O 优化 | `python-concurrency-reviewer` 关注并发安全（竞态、死锁） |
| 数据库查询 | 通用 I/O 和批量操作优化 | `python-database-reviewer` 关注 N+1、索引、连接池 |
| 内存泄漏 | 通用内存使用优化 | `python-error-handling-reviewer` 关注资源未关闭 |
...
# ┗━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━┛
#
# 场景：审查 Python 代码中的性能瓶颈
# 输入：项目代码文件
# 输出：性能审查报告（按严重程度分级）
#
# 审查步骤：
#   1. 使用 cProfile/memory_profiler 定位热点
#   2. 检查循环优化（列表推导式、预计算、join）
#   3. 检查数据结构选择（列表 vs 字典 vs deque）
#   4. 检查内存泄漏（无上限缓存、大对象持有）
#   5. 检查 I/O 优化（批量操作、异步替代）
#
# 常见问题模式：
#   - 嵌套循环: → 列表推导式或算法优化
#   - 字符串拼接: → str.join()
#   - 列表查找 O(n): → 字典查找 O(1)
#   - 无限缓存: → functools.lru_cache 或 LRUCache
#   - 同步阻塞 I/O: → 异步库 + asyncio.gather
#
# AI-Usage-End
```
